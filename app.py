...
# ----------------- Setup Players -----------------
if not st.session_state.player_setup_done and is_admin:
    ...
    st.stop()
elif not st.session_state.player_setup_done:
...
if not is_admin:
    st.markdown("🔒 You are in read-only mode. Login as admin to manage the game.")
    st.stop()
